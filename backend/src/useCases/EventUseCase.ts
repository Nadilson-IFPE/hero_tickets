...
class EventUseCase {
    ...
    async create(eventData: Event) {
        ...
        if (!eventData.flyers) {
            throw new HttpException(400, 'Flyer is required');
        }
        ...
        // Verificar se já existe um evento mo mesmo local e horário
        const verifyEvent = await this.eventRepository.findByLocationAnDate(
            eventData.location,
            eventData.date);
    ...
    private async getCityNameByCoordinates(latitude: string, longitude: string,) {
        try {
            const response = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=YOUR_API_KEY_HERE`);

            if (response.data.status === 'OK' && response.data.results.length > 0) {
                const address = response.data.results[0].address_components;
                const cityType = address.find((type: any) =>
            ...
            throw new HttpException(401, 'Error on requesting city name');
        }
    }
